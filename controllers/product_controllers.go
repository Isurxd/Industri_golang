...
func ReadDetailProducts(c echo.Context) (err error) {
	...
	SELECT
		products.id, product_name, products.price, categories.category_name, products.description
	FROM
		products
		INNER JOIN categories ON product.category_id = category_id = categories.id
	WHERE
		products.id = ?
	...
	return c.JSON(http.StatusOK, echo.Map{
		"message": "Successss all products!",
		"page":    nil,
		"data":    response,
		"error":   nil,
	})
...
func CreateProduct(c echo.Context) (err error) {
	...
	return c.JSON(http.StatusCreated, echo.Map{
		"message": "Succes creating data product!",
		"page":    nil,
		"data":    nil,
		"error":   nil,
	})
...
func DeleteProduct(c echo.Context) (err error) {
	...
	if err != nil {
		return c.JSON(http.StatusBadRequest, echo.Map{
			"message": "Failded  delete Product",
			"page":    nil,
			"data":    nil,
			"error":   err.Error(),
		})
